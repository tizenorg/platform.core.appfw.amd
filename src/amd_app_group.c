...
static void __prepare_to_suspend_services(int pid, uid_t uid)
{
	/*
	   ...
	   ret = aul_sock_send_raw_async(pid, uid, APP_SUSPEND,
...
static void __prepare_to_wake_services(int pid, uid_t uid)
{
	/*
	   ...
	   int dummy;
	   SECURE_LOGD("[__SUSPEND__] pid: %d", pid);
	   ret = aul_sock_send_raw_async(pid, uid, APP_WAKE,
	   (unsigned char *)&dummy, sizeof(int));
	   if (ret > 0)
	   close(ret);
	   */
	...
	while (list) {
		...
		if ((ac && ac->fg != flag) || force == TRUE) {
			...
			bg_category = (bool)appinfo_get_value(ai,
					AIT_BG_CATEGORY);
			_D("send signal %d %s", flag, appid);
			aul_send_app_status_change_signal(ac->pid, appid,
				pkgid,
				flag ? STATUS_FOREGROUND : STATUS_BACKGROUND,
				APP_TYPE_UI);
			if (!bg_category) {
				_status_find_service_apps(ac->pid, uid,
					flag ? STATUS_VISIBLE : STATUS_BG,
					__prepare_to_wake_services, false);
				/*
				   if (!flag) {
				   if (force == TRUE && ac->pid == cpid) {
				   __prepare_to_suspend_services(
				   ac->pid, uid);
				   _amd_suspend_add_timer(
				   ac->pid, ai);
				   }
				   }
				   */
