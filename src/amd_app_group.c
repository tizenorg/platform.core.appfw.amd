...
static void __prepare_to_suspend_services(int pid)
{
	...
	ret = aul_socket_send_raw_async(pid, getuid(), APP_SUSPEND, (unsigned char *)&dummy, sizeof(int));
	if (ret > 0)
		close(ret);
	*/
}
...
static void __prepare_to_wake_services(int pid)
{
	...
	int dummy;
	SECURE_LOGD("[__SUSPEND__] pid: %d", pid);
	ret = aul_socket_send_raw_async(pid, getuid(), APP_WAKE, (unsigned char *)&dummy, sizeof(int));
	if (ret > 0)
		close(ret);
	*/
}
