...
{
	if ((screen_mode == 90 || screen_mode == 270) && rotation.lock == true)
		...
		bundle *kb, int cmd)
{
	...
	if (__app_can_launch_splash_image(ai, kb, cmd) < 0)
        ...
        if (!rotation_initialized) {
                if (__init_rotation() < 0)
                        rotation.screen_mode = 0;
        }
	_D("screen_mode: %d", rotation.screen_mode);

	ai_ss = __get_splash_screen_info(ai, rotation.screen_mode);
	if (ai_ss == NULL)
		return NULL;
	...
	si->type = file_type;
	si->rotation = rotation.screen_mode;
                ...
                sensor_data_t *data, void *user_data)
{
        ...
        case AUTO_ROTATION_DEGREE_90:
                rotation.screen_mode = 90;
                break;
        case AUTO_ROTATION_DEGREE_180:
                rotation.screen_mode = 180;
                break;
        case AUTO_ROTATION_DEGREE_270:
                rotation.screen_mode = 270;
                break;
        default:
                break;
        }
...
{
        rotation.lock = vconf_keynode_get_bool(key);
        if (!rotation.lock) {
                rotation.screen_mode = 0;
                _D("lock: %d, screen_mode: %d",
                                rotation.lock, rotation.screen_mode);
        }
}
