	...
	for (iter = app_status_info_list; iter != NULL; iter = g_slist_next(iter)) {
		...
		if ((pid == info_t->pid) && ((info_t->uid == uid) || (info_t->uid == 0))) {
			...
			if (info_t->status == STATUS_VISIBLE) {
				info_t->resume_count++;
				if (info_t->pid != info_t->lpid)
					__update_leader_app_info(info_t->lpid);
				if (info_t->resume_count == 1
						&& limit_bg_uiapps > 0)
					__kill_bg_uiapps();
			}
...
static gint __sort_app_status_info_list(gconstpointer p1, gconstpointer p2)
{
	...
	if (info_t1->managed == false || info_t2->managed == false)
		return 0;
	...
	if (info_t1->timestamp > info_t2->timestamp) {
		...
		if (app_group_cnt1 > app_group_cnt2) {
			...
			if (info_t1->resume_count > info_t2->resume_count)
				return 1;
			else if (info_t1->resume_count < info_t2->resume_count)
				return -1;
			else
				return 0;
		}
