...
static void __add_widget_info(int viewer_pid, app_status_info_t *appinfo)
{
	...
	if (viewer_info_table == NULL) {
		viewer_info_table = g_hash_table_new(g_direct_hash,
				g_direct_equal);
		if (viewer_info_table == NULL) {
			_E("out of memory");
			return;
		...
		bool is_subapp, uid_t uid, int caller_pid)
{
	...
	if (component_type && (strcmp(component_type, APP_TYPE_WIDGET) == 0 ||
		...
		gconstpointer p2)
{
	...
	_app_group_get_group_pids(info_t1->lpid, &app_group_cnt1,
			&app_group_pids1);
	_app_group_get_group_pids(info_t2->lpid, &app_group_cnt2,
			&app_group_pids2);
