...
static int running_uiapps = 0;

static void __kill_bg_uiapps(void);
				...
				int pad_pid, bool is_subapp, int lpid, uid_t uid)
{
	...
	if (taskmanage && strcmp(taskmanage, "true") == 0
		...
		info_t->no_bgkill = false;
	else
		info_t->no_bgkill = true;

	app_status_info_list = g_slist_append(app_status_info_list, info_t);
	__add_pkg_info(pkgid, info_t);

	if (info_t->no_bgkill == true && info_t->status != STATUS_SERVICE) {
		__update_leader_app_info(lpid);
	} else if (info_t->no_bgkill == false) {
		running_uiapps++;
		if (limit_bg_uiapps > 0 && limit_bg_uiapps < running_uiapps)
			__kill_bg_uiapps();
	}
...
static gint __sort_app_status_info_list(gconstpointer p1, gconstpointer p2)
{
	...
	if (info_t1->no_bgkill == true || info_t2->no_bgkill == true)
		return 0;
	...
	if (info_t1->timestamp > info_t2->timestamp) {
		...
		app_group_get_group_pids(info_t1->lpid, &app_group_cnt1, &app_group_pids1);
		app_group_get_group_pids(info_t2->lpid, &app_group_cnt2, &app_group_pids2);
...
static void __kill_bg_uiapps(void)
{
	...
	GSList *iter_next;
	app_status_info_t *info_t;
	int n = running_uiapps - limit_bg_uiapps;
	int i = 0;

	if (n <= 0)
		return;

	app_status_info_list = g_slist_sort(app_status_info_list,
			(GCompareFunc)__sort_app_status_info_list);
	GSLIST_FOREACH_SAFE(app_status_info_list, iter, iter_next) {
		if (i == n)
			break;

		info_t = (app_status_info_t *)iter->data;
		if (info_t && info_t->no_bgkill == false
				&& info_t->status != STATUS_VISIBLE) {
