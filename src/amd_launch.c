		...
		struct launch_s *launch_data)
{
	...
	if (app_group_is_group_app(kb)) {
		launch_data->pid = -1;
		launch_data->is_subapp = true;
	} else {
		launch_data->is_subapp = false;
	}

	if (launch_data->pid > 0)
		status = _status_get_app_info_status(launch_data->pid, uid);

	if (launch_data->pid == -1 || status == STATUS_DYING) {
		ret = app_group_find_singleton(appid, &found_pid, &found_lpid);
		if (ret == 0) {
			launch_data->pid = found_pid;
			launch_data->new_process = false;
		} else {
			launch_data->new_process = true;
		}

		ret = app_group_can_start_app(appid, kb,
				&launch_data->can_attach,
				&launch_data->leader_pid,
				&launch_data->launch_mode);
		if (ret != 0) {
			_E("can't make group info");
			return -EILLEGALACCESS;
		}

		if (launch_data->can_attach &&
				launch_data->leader_pid == found_lpid) {
	...
	if (launch_data->pid == -1 && launch_data->can_attach)
		launch_data->pid = app_group_find_pid_from_recycle_bin(appid);
...
static int __prepare_starting_app(struct launch_s *launch_data, request_h req,
		const char *appid)
{
	int ret;
	const char *pkgid;
	...
	launch_data->appid = appid;
	launch_data->ai = appinfo_find(target_uid, appid);
	if (launch_data->ai == NULL) {
		_D("Failed to find appinfo of %s", appid);
		return -ENOENT;
	}

	ret = __check_executable(launch_data->ai);
	if (ret < 0)
		return -1;
	...
	ret = __compare_signature(launch_data->ai, cmd, target_uid, appid,
			caller_appid);
	if (ret < 0)
		return ret;

	comp_type = appinfo_get_value(launch_data->ai, AIT_COMPTYPE);
	if (comp_type == NULL)
		return -1;

	multiple = appinfo_get_value(launch_data->ai, AIT_MULTI);
	if (multiple == NULL || !strcmp(multiple, "false") == 0)
		launch_data->pid = _status_app_is_running(appid, target_uid);
	...
	if (strcmp(comp_type, APP_TYPE_UI) == 0) {
		...
		pkgid = appinfo_get_value(launch_data->ai, AIT_PKGID);
	...
	launch_data->share_handle = _temporary_permission_create(caller_pid,
			appid, kb, target_uid);
	if (launch_data->share_handle == NULL)
		_E("No sharable path: %d %s", caller_pid, appid);

	_extractor_mount(launch_data->ai, kb,
			_extractor_mountable_get_tep_paths(launch_data->ai));
	_extractor_mount(launch_data->ai, kb,
			_extractor_mountable_get_tpk_paths(launch_data->ai));

	launch_data->prelaunch_attr = __get_prelaunch_attribute(
			launch_data->ai);
	launch_data->bg_category = __get_background_category(launch_data->ai);
	launch_data->bg_allowed = __is_allowed_background(comp_type,
			launch_data->bg_category);
	if (launch_data->bg_allowed) {
...
static int __do_starting_app(struct launch_s *launch_data, request_h req,
		bool *pending)
{
	int status = -1;
	int cmd = _request_get_cmd(req);
	...
	if (launch_data->pid > 0) {
		status = _status_get_app_info_status(launch_data->pid,
				target_uid);
	}

	if (launch_data->pid > 0 && status != STATUS_DYING) {
		if (launch_data->pid == caller_pid) {
			SECURE_LOGD("caller & callee process are same. %s:%d,",
					launch_data->appid, launch_data->pid);
			return -ELOCALLAUNCH_ID;
		}

		aul_send_app_resume_request_signal(launch_data->pid,
				launch_data->appid, pkgid, comp_type);
		_suspend_remove_timer(launch_data->pid);
		if (comp_type && !strcmp(comp_type, APP_TYPE_SERVICE)) {
			if (launch_data->bg_allowed == false)
				__prepare_to_wake_services(launch_data->pid);
		}

		ret = __nofork_processing(cmd, launch_data->pid, kb, req);
		if (ret < 0) {
			_temporary_permission_destroy(
					launch_data->share_handle);
		}
		launch_data->pid = ret;
	...
	if (launch_data->pid > 0 && status == STATUS_DYING) {
		ret = kill(launch_data->pid, SIGKILL);
		if (ret == -1) {
			_W("Failed to send SIGKILL: %d:%s,", launch_data->pid,
					strerror(errno));
		}
		_cleanup_dead_info(launch_data->pid);
	}

	__set_appinfo_for_launchpad(launch_data->ai, kb);
	if (bundle_get_type(kb, AUL_K_SDK) != BUNDLE_TYPE_NONE)
		pad_type = DEBUG_LAUNCHPAD_SOCK;

	splash_image = _splash_screen_create_image(launch_data->ai, kb, cmd);
	_splash_screen_send_image(splash_image);

	_signal_send_proc_prelaunch(launch_data->appid, pkgid,
			launch_data->prelaunch_attr, launch_data->bg_category);

	ret = _send_cmd_to_launchpad(pad_type, target_uid, PAD_CMD_LAUNCH, kb);
	if (ret < 0) {
		_temporary_permission_destroy(launch_data->share_handle);
	...
	launch_data->pid = ret;
	*pending = true;
	_splash_screen_send_pid(splash_image, launch_data->pid);
	_suspend_add_proc(launch_data->pid);
	aul_send_app_launch_request_signal(launch_data->pid, launch_data->appid,
			pkgid, comp_type);
	if (comp_type && !strcmp(comp_type, APP_TYPE_SERVICE)) {
		if (launch_data->bg_allowed)
...
static int __complete_starting_app(struct launch_s *launch_data, request_h req)
{
	bundle *kb = _request_get_bundle(req);
	uid_t target_uid = _request_get_target_uid(req);
	const char *comp_type;
	...
	comp_type = appinfo_get_value(launch_data->ai, AIT_COMPTYPE);
	if (comp_type && !strcmp(comp_type, APP_TYPE_SERVICE)) {
		if (launch_data->new_process) {
			_D("Add app group info %d", launch_data->pid);
			__pid_of_last_launched_ui_app = launch_data->pid;
			app_group_start_app(launch_data->pid, kb,
					launch_data->leader_pid,
					launch_data->can_attach,
					launch_data->launch_mode);
			__add_fgmgr_list(launch_data->pid);
		} else {
			app_group_restart_app(launch_data->pid, kb);
		}
	}

	_status_add_app_info_list(launch_data->ai, launch_data->pid,
			launch_data->is_subapp, target_uid);

	if (launch_data->share_handle) {
		ret = _temporary_permission_apply(launch_data->pid, target_uid,
				launch_data->share_handle);
		if (ret < 0)
