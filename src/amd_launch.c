		...
		struct launch_s *launch_data)
{
	...
	if (launch_data->pid == -1 && launch_data->can_attach)
		launch_data->pid = _app_group_find_pid_from_recycle_bin(appid);
		...
		const char *appid)
{
	...
	launch_data->ai = _appinfo_find(target_uid, appid);
	...
	comp_type = _appinfo_get_value(launch_data->ai, AIT_COMPTYPE);
	if (comp_type == NULL)
		return -1;

	multiple = _appinfo_get_value(launch_data->ai, AIT_MULTI);
	...
	if (strcmp(comp_type, APP_TYPE_UI) == 0) {
		...
		pkgid = _appinfo_get_value(launch_data->ai, AIT_PKGID);
...
static int __complete_starting_app(struct launch_s *launch_data, request_h req)
{
	...
	if (comp_type && !strcmp(comp_type, APP_TYPE_SERVICE)) {
		if (launch_data->new_process) {
			...
			_app_group_start_app(launch_data->pid, kb,
					launch_data->leader_pid,
					launch_data->can_attach,
					launch_data->launch_mode);
			__add_fgmgr_list(launch_data->pid);
		} else {
			_app_group_restart_app(launch_data->pid, kb);
		}
	}
