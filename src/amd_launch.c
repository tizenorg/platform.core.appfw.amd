...
static int __get_visibility_from_certsvc(const char *cert_value, int *v)
{
	int ret;
...
static int __compare_certificate(const char *callee_pkgid,
	...
	if (launch_type == NULL
		...
		if (v == NULL) {
			cert_value = __get_cert_value_from_pkginfo(caller_pkgid,
					caller_uid);
			if (cert_value == NULL) {
				_E("Failed to get cert value %s", caller_pkgid);
				return 0;
			}

			ret = __get_visibility_from_certsvc(cert_value, &vi_num);
			if (ret == 0) {
		...
		uid_t caller_uid, int fd, bool *pending)
{
	...
	if (component_type
		...
		if (caller_appid) {
			ret = __compare_certificate(pkg_id,
