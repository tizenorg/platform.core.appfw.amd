...
static int __compare_signature(const struct appinfo *ai, int cmd,
		uid_t caller_uid, const char* appid, char *caller_appid,
		int fd)
{
		...
		bool *is_subapp)
{
	...
	if (pid == -1 || st == STATUS_DYING) {
		...
		if (app_group_can_start_app(appid, kb, caller_uid,
		...
		const char *caller_appid, uid_t caller_uid, bundle *kb)
{
	...
	if (launch_type == NULL
		...
		if (v == NULL) {
			cert_value = __get_cert_value_from_pkginfo(
					caller_pkgid, caller_uid);
		...
		request_h req, bool *pending)
{
	...
	if (component_type &&
		bg_category &&
		((strncmp(component_type, APP_TYPE_UI,
			strlen(APP_TYPE_UI)) == 0)
		|| (strncmp(component_type, APP_TYPE_SERVICE,
			strlen(APP_TYPE_SERVICE)) == 0))) {
		bg_allowed = true;
	} else if (component_type &&
			((strncmp(component_type, APP_TYPE_WATCH,
				strlen(APP_TYPE_WATCH)) == 0)
			|| (strncmp(component_type, APP_TYPE_WIDGET,
				strlen(APP_TYPE_WIDGET)) == 0))) {
		bg_allowed = true;
	} else {
		bg_allowed = false;
	}
	...
	if (bg_allowed) {
		_D("[__SUSPEND__] allowed background, appid: %s, "
		   "bg category: 0x%x, app type: %s, api version: %s",
	...
	if (pid > 0 && callee_status != STATUS_DYING) {
		...
		_signal_send_proc_prelaunch(appid, pkg_id,
				prelaunch_attribute, bg_category);
		...
		if (pid > 0) {
			*pending = true;
			...
			aul_send_app_launch_request_signal(pid, appid,
