...
int _resume_app(int pid, int clifd)
{
	...
	if ((ret = aul_socket_send_raw_async(pid, getuid(), APP_RESUME_BY_PID,
...
int _pause_app(int pid, int clifd)
{
	...
	if ((ret = aul_socket_send_raw_async(pid, getuid(), APP_PAUSE_BY_PID,
			(unsigned char *)&dummy, 0)) < 0) {
		if (ret == -EAGAIN)
...
int _term_sub_app(int pid)
{
	...
	if ((ret = aul_socket_send_raw_async(pid, getuid(), APP_TERM_BY_PID_ASYNC,
			(unsigned char *)&dummy, 0)) < 0) {
...
int _term_app(int pid, int clifd)
{
	...
	if ((ret = aul_socket_send_raw_async(pid, getuid(), APP_TERM_BY_PID,
			(unsigned char *)&dummy, 0)) < 0) {
...
int _term_req_app(int pid, int clifd)
{
	...
	if ((ret = aul_socket_send_raw_async(pid, getuid(), APP_TERM_REQ_BY_PID,
			(unsigned char *)&dummy, 0)) < 0) {
...
int _term_bgapp(int pid, int clifd)
{
	...
	if ((fd = aul_socket_send_raw_async(pid, getuid(), APP_TERM_BGAPP_BY_PID,
...
int _fake_launch_app(int cmd, int pid, bundle *kb, int clifd)
{
	...
	if ((ret = aul_socket_send_raw_async(pid, getuid(), cmd, kb_data, datalen)) < 0) {
