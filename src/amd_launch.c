...
static gboolean __launchimage_timeout_handler(gpointer data)
{
	struct tizen_launch_image *launchimage = (struct tizen_launch_image *)data;

	if (launchimage == NULL)
		return FALSE;

	tizen_launch_image_destroy(launchimage);
...
static int __app_can_launch_image(const struct appinfo *ai, bundle *b, int cmd)
{
	...
	/* TODO: Fix the exception for third pary */
	if (cmd != APP_OPEN) {
				...
				int screen_mode, char *file, int size)
{
	...
	/*
	 * TODO: Support splash screen for third party
	 */
...
static struct tizen_launch_image *__send_image_to_wm(const struct appinfo *ai,
					bundle *b, int cmd)
{
	struct tizen_launch_image *launchimage;
	guint tid;
	...
	launchimage = tizen_launchscreen_create_img(launchscreen);
	if (launchimage == NULL)
		return NULL;

	wl_display_flush(display);
	tizen_launch_image_launch(launchimage, image_file,
				file_type, indicator, screen_mode);
	tid = g_timeout_add(3000, __launchimage_timeout_handler,
				(gpointer)launchimage);
	if (tid <= 0)
...
static void __send_pid_to_wm(struct tizen_launch_image *launchimage, int pid)
{
	if (launchimage == NULL)
		return;

	tizen_launch_image_owner(launchimage, pid);
}
...
int _launch_init()
{
	...
	*/
	...
	if (wl_display_roundtrip(display) < 0)
		_E("Failed to process wayland connection");
		...
		uid_t caller_uid, int fd, bool *pending)
{
	...
	if (pid > 0 && callee_status != STATUS_DYING) {
		...
		launchimage = __send_image_to_wm(ai, kb, cmd);

		pid = _send_cmd_to_launchpad(pad_type, caller_uid, PAD_CMD_LAUNCH, kb);
		if (pid > 0) {
			*pending = true;
			__send_pid_to_wm(launchimage, pid);
