...
extern void _cleanup_dead_info(app_status_h stat);
		...
		struct launch_s *handle)
{
	...
	if (handle->pid > 0) {
		app_status = _app_status_find(handle->pid);
		status = _app_status_get_status(app_status);
	}
...
static gboolean __check_service_only(gpointer user_data)
{
	...
	_app_status_check_service_only(app_status, __prepare_to_suspend_services);
...
static gboolean __fg_timeout_handler(gpointer data)
{
	...
	stat = _app_status_find(fg->pid);
	_app_status_update_status(stat, STATUS_BG, true);
...
static int __app_status_handler(int pid, int status, void *data)
{
	...
	stat = _app_status_find(pid);
	if (stat == NULL)
		return 0;

	app_statusus  = _app_status_get_status(stat);
	if (app_statusus == STATUS_DYING && status != PROC_STATUS_LAUNCH)
		return 0;

	switch (status) {
	case PROC_STATUS_FG:
		__del_fgmgr_list(pid);
		_app_status_update_status(stat, STATUS_VISIBLE, false);
		...
		break;
	case PROC_STATUS_BG:
		_app_status_update_status(stat, STATUS_BG, false);
		appid = _app_status_get_appid(stat);
		if (appid) {
		...
		bool *pending)
{
	...
	if (handle->pid > 0) {
		stat = _app_status_find(handle->pid);
		status = _app_status_get_status(stat);
	}
