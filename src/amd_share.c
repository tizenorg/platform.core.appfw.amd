...
{
	share_file_info_h handle = malloc(sizeof(struct share_file_info_s));

	if (handle == NULL)
		return NULL;

	((struct share_file_info_s *)handle)->caller_exec_label = caller_exec_label;
	((struct share_file_info_s *)handle)->callee_exec_label = callee_exec_label;
	((struct share_file_info_s *)handle)->paths = paths;
...
share_file_info_h _grant_temporary_permission(int caller_pid, const char *appid, bundle *kb)
{
	...
	if (valid && caller_exec_label && paths) {
		...
		if (r != SECURITY_SERVER_API_SUCCESS) {
			...
			share_file_info_h h = __new_share_file_info(caller_exec_label, callee_exec_label, paths);
			if (h)
				return h;
		//}
	}
...
int _register_temporary_permission(int pid, share_file_info_h handle)
{
	if (handle) {
		...
		free(h->caller_exec_label);
		free(h->callee_exec_label);
		free(h);
		return 0;
	}
