...
static const char *__get_owner_appid(int caller_pid, bundle *kb)
{
	...
	app_stat = _app_stat_find(owner_pid);
	owner_appid = _app_stat_get_appid(app_stat);
...
int _temporary_permission_apply(int pid, uid_t uid, shared_info_h handle)
{
	...
	ret = _app_stat_add_shared_info(app_stat, handle->shared_info);
	if (ret != 0)
		return ret;
...
int _temporary_permission_drop(int pid, uid_t uid)
{
	...
	list = _app_stat_get_shared_info_list(app_stat);
	if (!list) {
		_D("list was null");
		return -1;
	}
	...
	return _app_stat_clear_shared_info_list(app_stat);
}
