...
static int __can_share(const char *path, const char *pkgid)
{
	...
	snprintf(buf, sizeof(buf), "/home/owner/apps_rw/%s/data/", pkgid);
...
shared_info_h _temporary_permission_create(int caller_pid, const char *appid, bundle *kb)
{
	...
	if (__has_valid_uri(caller_pid, appid, kb, paths, &owner_pid))
		valid = true;
finally:
...
shared_info_h _temporary_permission_apply(int pid, shared_info_h handle)
{
	if (handle) {
		_status_add_shared_info(pid, getuid(), handle);
		return NULL;
	}
