...
static char **__get_tep_paths(const struct appinfo *ai)
{
...
static char **__get_tpk_paths(const struct appinfo *ai)
{
...
static char **__get_mount_paths(const struct appinfo *ai, int type)
{
	switch (type) {
	case EX_TEP:
		return __get_tep_paths(ai);
	case EX_TPK:
		return __get_tpk_paths(ai);
	default:
		return NULL;
	}
}
...
void _extractor_mount(const struct appinfo *ai, bundle *kb, int type)
{
	...
	mnt_path = __get_mount_paths(ai, type);
	if (mnt_path == NULL)
		return;
