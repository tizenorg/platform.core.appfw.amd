...
int _wayland_add_registry_listener(struct wl_registry_listener *listener,
		void *data)
{
		...
		unsigned int id, const char *interface, unsigned int version)
{
	...
	for (iter = g_list_first(wl_list); iter; iter = g_list_next(iter)) {
		...
		if (listen && listen->listener && listen->listener->global)
			listen->listener->global(listen->data, reg, id,
					interface, version);
	}
}
		...
		unsigned int id)
{
	...
	while (iter) {
		...
		if (listen && listen->listener &&
				listen->listener->global_remove)
			listen->listener->global_remove(listen->data, reg, id);
