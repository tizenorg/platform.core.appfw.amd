		...
		bool *pending)
{
	...
	case APP_KILL_BY_PID:
		...
		_app_stat_update_status(app_stat, STATUS_DYING, false);
...
static int __dispatch_app_result(request_h req)
{
	...
	app_stat = _app_stat_find(getpgid(pid));
	appid = _app_stat_get_appid(app_stat);
	if (appid) {
...
static int __dispatch_app_pause(request_h req)
{
	...
	appid = bundle_get_val(kb, AUL_K_APPID);
	app_stat = _app_stat_find_by_appid(appid, _request_get_target_uid(req));
	ret = _app_stat_is_running(app_stat);
	if (ret > 0)
...
static int __dispatch_app_term_async(request_h req)
{
	...
	app_stat = _app_stat_find(atoi(term_pid));
	appid = _app_stat_get_appid(app_stat);
...
static int __dispatch_app_running_info(request_h req)
{
	...
	ret = _app_stat_send_running_appinfo(_request_remove_fd(req),
...
static int __dispatch_app_get_appid_by_pid(request_h req)
{
	...
	ret = _app_stat_get_appid_bypid(_request_remove_fd(req), pid);
	_D("app_stat_get_appid_bypid : %d : %d", pid, ret);
...
static int __dispatch_app_get_pkgid_by_pid(request_h req)
{
	...
	ret = _app_stat_get_pkgid_bypid(_request_remove_fd(req), pid);
...
static int __dispatch_app_status_update(request_h req)
{
	...
	app_stat = _app_stat_find(req->pid);
	if (app_stat == NULL)
		return -1;
	...
	if (*status == STATUS_NORESTART) {
		...
		_app_stat_update_status(app_stat, *status, false);
	}
...
static int __dispatch_agent_dead_signal(request_h req)
{
	...
	_app_stat_remove_all_app_info_with_uid(target_uid);
...
static int __dispatch_app_register_pid(request_h req)
{
	...
	_app_stat_add_app_info(ai, pid, false, target_uid, caller_pid);
...
static int __dispatch_app_set_process_group(request_h req)
{
	...
	app_stat = _app_stat_find(child_pid);
	child_appid = _app_stat_get_appid(app_stat);
	if (child_appid == NULL) {
		_E("No child appid");
		_request_send_result(req, -1);
		return -1;
...
static int __dispatch_app_get_status_by_appid(request_h req)
{
	...
	pid = _app_stat_is_running(app_stat);
	if (pid <= 0) {
		_request_send_result(req, -1);
		return -1;
	}
...
static int __check_app_is_running(request_h req)
{
	...
	case APP_TERM_BY_PID_SYNC:
		...
		break;
	default:
		app_stat = _app_stat_find_by_appid(str,
	...
	return _app_stat_get_pid(app_stat);
}
