	...
	if (req->uid < REGULAR_UID_MIN) {
		...
		if (target_uid != NULL) {
			...
			if (strcmp(state, "offline") &&
				...
				ret = _start_app(appid, kb, req->cmd, req->pid,
		...
		ret = _start_app(appid, kb, req->cmd, req->pid, req->uid, req->clifd,
				&pending);
	}
		...
		struct ucred cr)
{
	...
	req = (request_s *)malloc(sizeof(request_s) + pkt->len + 1);
	if (req == NULL)
		return NULL;
