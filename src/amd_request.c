...
static int __dispatch_app_pause(request_h req)
{
	...
	appid = bundle_get_val(kb, AUL_K_APPID);
	app_status = _app_status_find_by_appid(appid, _request_get_target_uid(req));
...
static int __dispatch_app_statusus_update(request_h req)
{
	int *status;
	const char *appid;
	struct appinfo *ai;
...
static app_cmd_dispatch_func dispatch_table[APP_CMD_MAX] = {
	...
	[APP_STATUS_UPDATE] = __dispatch_app_statusus_update,
