...
static int __app_process_by_pid(request_h req, const char *pkg_name)
{
	int pid;
	int ret;
	int dummy;
	...
	case APP_RESUME_BY_PID:
		ret = _resume_app(pid, req->clifd);
		req->clifd = 0;
		break;
	case APP_TERM_BY_PID:
	case APP_TERM_BY_PID_WITHOUT_RESTART:
		ret = _term_app(pid, req->clifd);
		req->clifd = 0;
		break;
	case APP_TERM_BGAPP_BY_PID:
		ret = _term_bgapp(pid, req->clifd);
		req->clifd = 0;
		break;
	case APP_KILL_BY_PID:
		if ((ret = _send_to_sigkill(pid)) < 0)
			_E("fail to killing - %d\n", pid);
		_status_update_app_info_list(pid, STATUS_DYING, FALSE, uid);
		_request_send_result(req, ret);
		break;
	case APP_TERM_REQ_BY_PID:
		ret = _term_req_app(pid, req->clifd);
		req->clifd = 0;
		break;
...
static void __handle_agent_dead_signal(uid_t uid)
{
	/* TODO: check the credentials from the caller: must be the amd agent */

	_D("AGENT_DEAD_SIGNAL : %d", uid);
	__agent_dead_handler(uid);
}
...
static int __dispatch_app_start(request_h req)
{
	...
	if (req->uid < REGULAR_UID_MIN) {
		...
		if (target_uid != NULL) {
			...
			if (strcmp(state, "offline") &&
				...
				ret = _start_app(appid, kb, req->pkt->cmd, req->pid,
						t_uid, req->clifd, &pending);
			} else {
			...
			ret = _start_app(appid, kb, req->pkt->cmd, req->pid,
					GLOBAL_USER, req->clifd, &pending);
		}
	} else {
		ret = _start_app(appid, kb, req->pkt->cmd, req->pid, req->uid, req->clifd,
				&pending);
	}
	...
	if (ret > 0 && __add_rua_info(req->pkt, kb, req->uid, appid) < 0)
		goto error;
	return 0;
...
static int __dispatch_app_result(request_h req)
{
	...
	if ((res = aul_sock_send_bundle_async(pid, getuid(), cmd, kb, AUL_SOCK_CLOSE | AUL_SOCK_NOREPLY)) < 0)
		res = AUL_R_ERROR;
...
{
	_status_send_running_appinfo(req->clifd, req->uid);
	req->clifd = 0;
...
static int __dispatch_app_get_appid_by_pid(request_h req)
{
	...
	ret = _status_get_appid_bypid(req->clifd, pid);
...
static int __dispatch_app_get_pkgid_by_pid(request_h req)
{
	...
	ret = _status_get_pkgid_bypid(req->clifd, pid);
...
static int __dispatch_agent_dead_signal(request_h req)
{
	...
	__handle_agent_dead_signal(req->uid);
...
static void __process_pending_request(gpointer data, gpointer user_data)
{
	...
	dispatch_table[req->pkt->cmd](req);
...
static gboolean __timeout_pending_item(gpointer user_data)
{
	...
	_request_send_result(item->req, item->pid);
		...
		struct ucred cr)
{
	...
	req = (request_s *)malloc(sizeof(request_s));
	if (req == NULL)
		return NULL;
...
{
	aul_sock_send_raw_async_with_fd(req->clifd, cmd, data, len, AUL_SOCK_CLOSE);
	req->clifd = 0;
	return 0;
} 
...
{
	_send_result_to_client(req->clifd, res);
	req->clifd = 0;
	return 0;
}
